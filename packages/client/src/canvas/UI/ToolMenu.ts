...
import type { CaddieMenu } from "./CaddieMenu";
...
export class ToolMenu {
  ...
  private buttonImgs: HTMLElement[];
  private toolbar: HTMLDivElement;

  constructor(toolManager: ToolManager, caddieMenu: CaddieMenu) {
    this.toolManager = toolManager;
    this.caddieMenu = caddieMenu;
    ...
    toolManager.selectPen(hexToRGB(this.penColor), this.penWidth, 1);
  }
  ...
  createToolbar() {
    ...
    toolbar.append(
      this.createToolButton(Icons.Select, () => {
    ...
    toolbar.append(
      this.createToolButton(Icons.Eraser, () => {
        this.currentTool = ToolTypes.ERASER;
        this.toolManager.enableEraser();
      })
    );
    toolbar.append(this.createUndoButton());
  ...
  createPenButton(
    ...
    if (preselected) {
      penImg.style.border = SELECTED_ICON_BORDER;
    }
    ...
    const updateIndicators = () => {
      console.log(sortedWidths, getWidth());
    ...
    const showWidthBar = (colorIcon, color) => {
      ...
      widthIcons = otherWidths.map((width) => {
        return this.generatePenColorSizeIcon(color, width, widths);
      });
    ...
    const handlePointer = ({ x, y }) => {
      ...
      if (x > colorIcons[0][0].getBoundingClientRect().left) {
        let i = 0;
        while (i < colorIcons.length - 1 && y >= colorIcons[i + 1][0].getBoundingClientRect().top) {
          i++;
        }
        ...
        let i = 0;
        while (i < widthIcons.length - 1 && x >= widthIcons[i + 1][0].getBoundingClientRect().left) {
          i++;
        }
    ...
    penBtn.addEventListener("pointerdown", (e) => {
      ...
      colorIcons = colorsList.map((color, ix) => {
        return this.generatePenColorSizeIcon(color, getWidth(), widths);
      });
  ...
  createUndoButton() {
    const [btn, img] = this.createButton(Icons.Undo);
  ...
  createToolButton(iconName, selectFunc) {
    const [btn, img] = this.createButton(iconName);
  ...
  createButton(icon: HTMLElement): [HTMLElement, HTMLElement] {
    ...
    this.buttonImgs.push(icon);

    return [btn, icon];
  }

  resetButtonsOutline() {
    this.buttonImgs.forEach((img) => {
      img.style.border = UNSELECTED_ICON_BORDER;
    });
  }

  generatePenColorSizeIcon(color: string, size: number, widths: number[]) {
    ...
    const updateSize = (newSize: number) => {
      ...
      icon.style.borderRadius = `${Math.ceil(s / 2)}px`;
      outline.style.width = `${Math.ceil(s + 4)}px`;
      outline.style.height = `${Math.ceil(s + 4)}px`;
      outline.style.borderRadius = `${Math.ceil(s + 3)}px`;
    };
    updateSize(size);
  ...
  createDragHandle() {
    ...
    handle.addEventListener("pointermove", (e) => {
      if (dragging) {
        ...
        this.caddieMenu.translatePosition(V2.sub(e, { x, y }));
  ...
  changeHighlighterColor(color: string) {
    ...
    console.log("Highlighter color changed: " + this.highlighterColor);
  }
