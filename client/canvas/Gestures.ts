...
export class ViewManager {
  ...
  inertia: ScrollInertia;
  averagePointerPos: { x: number; y: number };
  averagePointerDist: number;
  touches: any[];
  ...
  update(touches) {
    ...
    if (N > 0) {
      ...
      xAvg /= N;
      yAvg /= N;
      for (let { x, y } of Object.values(this.touches)) rAvg += Math.sqrt((x - xAvg) ** 2 + (y - yAvg) ** 2);
  ...
  handleTouchEvent(e) {
    ...
    if (type != "touchmove" || touches.length != this.touches.length) {
      // Touch started/ended
      this.update(touches);
      if (touches.length) this.inertia.reset(timeStamp);
    } else {
