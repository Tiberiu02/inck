...
function ComputeSmoothPath(s, full) {
  if (s.length == ELEMENTS_PER_INPUT) return [{ x: s[0], y: s[1], t: 0, nx: 1, ny: 0 }];
  ...
  while (t < s[s.length - 1]) {
    while (t >= s[i + ELEMENTS_PER_INPUT + OFFSET_INPUT.T]) i += ELEMENTS_PER_INPUT;
...
export function StrokeToPath(s: number[], w: number, color: number[], full: boolean = true): number[] {
  ...
  let lenSmooth = 0;
  for (let i = 1; i < path.length; i++) lenSmooth += Math.sqrt((path[i].x - path[i - 1].x) ** 2 + (path[i].y - path[i - 1].y) ** 2);

  let inputsByDist = [{ p: s[OFFSET_INPUT.P], d: 0 }];
  let lenRaw = 0;
  for (let i = ELEMENTS_PER_INPUT; i < s.length; i += ELEMENTS_PER_INPUT) {
    lenRaw += Math.sqrt(
      (s[i - ELEMENTS_PER_INPUT + OFFSET_INPUT.X] - s[i + OFFSET_INPUT.X]) ** 2 +
        (s[i - ELEMENTS_PER_INPUT + OFFSET_INPUT.Y] - s[i + OFFSET_INPUT.Y]) ** 2
    );
  ...
  for (let i = 0; i < path.length; i++) {
    ...
    if (i > 0) currLen += Math.sqrt((x - path[i - 1].x) ** 2 + (y - path[i - 1].y) ** 2);

    while (closestInput + 1 < inputsByDist.length && (currLen / lenSmooth) * lenRaw > inputsByDist[closestInput].d) closestInput += 1;

    const p =
      closestInput > 0
        ? interpolate(
            inputsByDist[closestInput - 1].d,
            inputsByDist[closestInput - 1].p,
            inputsByDist[closestInput].d,
            inputsByDist[closestInput].p,
            (currLen / lenSmooth) * lenRaw
          )
        : inputsByDist[closestInput].p;
    const r = GetR(p);
