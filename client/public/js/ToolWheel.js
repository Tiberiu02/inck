...
export default class ToolWheel {
  ...
  NewTool() {
    const rgb = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(this.color).slice(1).map(v => parseInt(v, 16) / 255)

    if (this.tool == 'pen') {
      return new Pen(DPI * WIDTHS[this.width.pen] / innerWidth / this.app.view.zoom, [...rgb, 1])
    } else if (this.tool == 'highlighter') {
      return new Pen(DPI * H_WIDTHS[this.width.highlighter] / innerWidth / this.app.view.zoom, [...rgb, 0.2])
    } else if (this.tool == 'eraser') {
      return new Eraser();
    } else if (this.tool == 'shapes') {
      return new Pen(DPI * WIDTHS[this.width.shapes] / innerWidth / this.app.view.zoom, [...rgb, 1])
    } else if (this.tool == 'selection')
      return new Pen(DPI * 0.02 / innerWidth / this.app.view.zoom, [0,0,0, 1])
  }
  ...
  static build(R, actions) {
    ...
    // Selection
    {
      ...
      const iconSize = R / 7
      const ai = a
      selection.appendChild(createElement('image', {
    ...
    function AddOptionButton(img, angle, action) {
      ...
      option.appendChild(createElement('circle', {
        cx: x, cy: y, r: iconSize * 0.8
      }));
      
      option.appendChild(createElement('image', {
        href: img,
        transform: `translate(${-iconSize / 2}, ${-iconSize / 2})`,
        height: iconSize, width: iconSize,
        x: x, y: y
  ...
  static buildWidthsWheel(R, type, setW) {
    ...
    for (let i = 0; i < N_WIDTHS; i++) {
      ...
      if (type == 'highlighter') {
        const [x, y] = spin(R * .8, a);
        g.appendChild(createElement('circle', {
          cx: x, cy: y,
        ...
        let path2 = `
          ...
          Q ${spin(R * 0.88, a1 * .22 + a2 * .78)} ${spin(R * 0.7, a)}
